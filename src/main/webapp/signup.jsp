...
$(function() {
	...
	$('#id').blur(function() {
		...
		// 입력 id 값이 정규식에 벗어날 경우
			...
			$('#id2check').html('<h6 style=color:red;>영문 대소문자, 숫자 4~12자리로 구성되어야 합니다.</h6>')
		}
	...
	$('#tel').blur(function() {
		...
		// 입력한 tel 값이 전화번호 정규식에 포함될 경우
		if (result != null) {
			$('#telcheck').html('')
			
		// 입력한 tel 값이 전화번호 정규식에 벗어날 경우	
		} else {
	...
	$('#email').blur(function() {
        ...
        result = regex.exec($("#email").val());
     	...
     	// 입력한 email 값이 이메일주소 정규식에 포함될 경우
		if (result != null) {
			$('#emailcheck').html('')
		
		// 입력한 email 값이 이메일주소 정규식에서 벗어날 경우
		} else {
...
//  생년월일 유효성 검사 초기화
	...
	//생년월일 유효성 검사
	$('#birth').blur(function(){
		dateStr = $(this).val(); // 브라우저에서 입력한 생년월일 글자 수
